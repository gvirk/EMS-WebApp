	...
	<div class="form-container">
		...
		<div class="row">
			<div class="col-md-4 col-sm-4 col-xs-12">
				<div class="form-group">
					...
					<input type="text" class="form-control" id="incomings" value="${theUser.marchSalary}">
				</div>
			</div>
			<div class="col-md-4 col-sm-4 col-xs-12">
				<div class="form-group">
					<label>Total outgoings:</label> 
					<input type="text" class="form-control" id="outgoings" value="${theSumMar[0]+theSumMar[1]+theSumMar[2]+theSumMar[3]+theSumMar[4]+theSumMar[5]+theSumMar[6]}">
				</div>
			</div>
			<div class="col-md-4 col-sm-4 col-xs-12">
				<div class="form-group">
					<label>Save Up:</label> 
					<input type="text" class="form-control" style="color: red; font-weight: bold" id="result" value="${theUser.marchSalary-theSumMar[0]-theSumMar[1]-theSumMar[2]-theSumMar[3]-theSumMar[4]-theSumMar[5]-theSumMar[6]}">
				</div>
			</div>
		</div>
		</c:forEach>
        ...
        <div class="row">
	        ...
	        <div class="col-md-2 col-sm-2 col-xs-12">
		        <div class="form-group">
             		<input type="button" value="Add new expends" class="btn btn-success btn-block" onclick="window.location.href='showFormForAddExpendsMar'; return false;"/>
		        </div>
	        </div>
		</div>
		...
		<div class="table-responsive center">
			<table class="table table-hover table-sm table-bordered">
				...
				<c:forEach var="tempMarExp" items="${marchExpends}">
				
					<!-- construct an "update" link with row id -->
					<c:url var="updateLink" value="/user/showExpForUpdateMar">
						<c:param name="rowId" value="${tempMarExp.id}" />
					</c:url>
					
					<!-- construct an "delete" link with row id -->
					<c:url var="deleteLink" value="/user/deleteRowMar">
						<c:param name="rowId" value="${tempMarExp.id}" />
					</c:url>
				
					<tr>
						<td>${tempMarExp.id}</td>
						<td>${tempMarExp.date}</td>
						<td>${tempMarExp.eating}</td>
						<td>${tempMarExp.mobile}</td>
						<td>${tempMarExp.flat}</td>
						<td>${tempMarExp.fuel}</td>
						<td>${tempMarExp.tickets}</td>
						<td>${tempMarExp.payment}</td>
						<td>${tempMarExp.other}</td>
						<td><strong>${tempMarExp.total=tempMarExp.eating+tempMarExp.mobile+tempMarExp.flat+tempMarExp.fuel+tempMarExp.tickets+tempMarExp.payment+tempMarExp.other}</strong></td>
					...
					<tr class="background">
						...
						<td><strong>${theSumMar[0]}</strong></td>
						<td><strong>${theSumMar[1]}</strong></td>
						<td><strong>${theSumMar[2]}</strong></td>
						<td><strong>${theSumMar[3]}</strong></td>
						<td><strong>${theSumMar[4]}</strong></td>
						<td><strong>${theSumMar[5]}</strong></td>
						<td><strong>${theSumMar[6]}</strong></td>
						<td><strong>${theSumMar[0]+theSumMar[1]+theSumMar[2]+theSumMar[3]+theSumMar[4]+theSumMar[5]+theSumMar[6]}</strong></td>
						<td colspan="2"></td>
					</tr>
