	...
	<div class="form-container">
		...
		<div class="row">
			<div class="col-md-4 col-sm-4 col-xs-12">
				<div class="form-group">
					...
					<input type="text" class="form-control" id="incomings" value="${theUser.octoberSalary}">
				</div>
			</div>
			<div class="col-md-4 col-sm-4 col-xs-12">
				<div class="form-group">
					<label>Total outgoings:</label> 
					<input type="text" class="form-control" id="outgoings" value="${theSumOct[0]+theSumOct[1]+theSumOct[2]+theSumOct[3]+theSumOct[4]+theSumOct[5]+theSumOct[6]}">
				</div>
			</div>
			<div class="col-md-4 col-sm-4 col-xs-12">
				<div class="form-group">
					<label>Save Up:</label> 
					<input type="text" class="form-control" style="color: red; font-weight: bold" id="result" value="${theUser.octoberSalary-theSumOct[0]-theSumOct[1]-theSumOct[2]-theSumOct[3]-theSumOct[4]-theSumOct[5]-theSumOct[6]}">
				</div>
			</div>
		</div>
		</c:forEach>
        ...
        <div class="row">
	        ...
	        <div class="col-md-2 col-sm-2 col-xs-12">
		        <div class="form-group">
             		<input type="button" value="Add new expends" class="btn btn-success btn-block" onclick="window.location.href='showFormForAddExpendsOct'; return false;"/>
		        </div>
	        </div>
		</div>
		...
		<div class="table-responsive center">
			<table class="table table-hover table-sm table-bordered">
				...
				<c:forEach var="tempOctExp" items="${octoberExpends}">
				
					<!-- construct an "update" link with row id -->
					<c:url var="updateLink" value="/user/showExpForUpdateOct">
						<c:param name="rowId" value="${tempOctExp.id}" />
					</c:url>
					
					<!-- construct an "delete" link with row id -->
					<c:url var="deleteLink" value="/user/deleteRowOct">
						<c:param name="rowId" value="${tempOctExp.id}" />
					</c:url>
				
					<tr>
						<td>${tempOctExp.id}</td>
						<td>${tempOctExp.date}</td>
						<td>${tempOctExp.eating}</td>
						<td>${tempOctExp.mobile}</td>
						<td>${tempOctExp.flat}</td>
						<td>${tempOctExp.fuel}</td>
						<td>${tempOctExp.tickets}</td>
						<td>${tempOctExp.payment}</td>
						<td>${tempOctExp.other}</td>
						<td><strong>${tempOctExp.total=tempOctExp.eating+tempOctExp.mobile+tempOctExp.flat+tempOctExp.fuel+tempOctExp.tickets+tempOctExp.payment+tempOctExp.other}</strong></td>
					...
					<tr class="background">
						...
						<td><strong>${theSumOct[0]}</strong></td>
						<td><strong>${theSumOct[1]}</strong></td>
						<td><strong>${theSumOct[2]}</strong></td>
						<td><strong>${theSumOct[3]}</strong></td>
						<td><strong>${theSumOct[4]}</strong></td>
						<td><strong>${theSumOct[5]}</strong></td>
						<td><strong>${theSumOct[6]}</strong></td>
						<td><strong>${theSumOct[0]+theSumOct[1]+theSumOct[2]+theSumOct[3]+theSumOct[4]+theSumOct[5]+theSumOct[6]}</strong></td>
						<td colspan="2"></td>
					</tr>
