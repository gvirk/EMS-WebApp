	...
	<div class="form-container">
		...
		<div class="row">
			<div class="col-md-4 col-sm-4 col-xs-12">
				<div class="form-group">
					...
					<input type="text" class="form-control" id="incomings" value="${theUser.aprilSalary}">
				</div>
			</div>
			<div class="col-md-4 col-sm-4 col-xs-12">
				<div class="form-group">
					<label>Total outgoings:</label> 
					<input type="text" class="form-control" id="outgoings" value="${theSumApr[0]+theSumApr[1]+theSumApr[2]+theSumApr[3]+theSumApr[4]+theSumApr[5]+theSumApr[6]}">
				</div>
			</div>
			<div class="col-md-4 col-sm-4 col-xs-12">
				<div class="form-group">
					<label>Save Up:</label> 
					<input type="text" class="form-control" style="color: red; font-weight: bold" id="result" value="${theUser.aprilSalary-theSumApr[0]-theSumApr[1]-theSumApr[2]-theSumApr[3]-theSumApr[4]-theSumApr[5]-theSumApr[6]}">
				</div>
			</div>
		</div>
		</c:forEach>
        ...
        <div class="row">
	        ...
	        <div class="col-md-2 col-sm-2 col-xs-12">
		        <div class="form-group">
             		<input type="button" value="Add new expends" class="btn btn-success btn-block" onclick="window.location.href='showFormForAddExpendsApr'; return false;"/>
		        </div>
	        </div>
		</div>
		...
		<div class="table-responsive center">
			<table class="table table-hover table-sm table-bordered">
				...
				<c:forEach var="tempAprExp" items="${aprilExpends}">
				
					<!-- construct an "update" link with row id -->
					<c:url var="updateLink" value="/user/showExpForUpdateApr">
						<c:param name="rowId" value="${tempAprExp.id}" />
					</c:url>
					
					<!-- construct an "delete" link with row id -->
					<c:url var="deleteLink" value="/user/deleteRowApr">
						<c:param name="rowId" value="${tempAprExp.id}" />
					</c:url>
				
					<tr>
						<td>${tempAprExp.id}</td>
						<td>${tempAprExp.date}</td>
						<td>${tempAprExp.eating}</td>
						<td>${tempAprExp.mobile}</td>
						<td>${tempAprExp.flat}</td>
						<td>${tempAprExp.fuel}</td>
						<td>${tempAprExp.tickets}</td>
						<td>${tempAprExp.payment}</td>
						<td>${tempAprExp.other}</td>
						<td><strong>${tempAprExp.total=tempAprExp.eating+tempAprExp.mobile+tempAprExp.flat+tempAprExp.fuel+tempAprExp.tickets+tempAprExp.payment+tempAprExp.other}</strong></td>
					...
					<tr class="background">
						...
						<td><strong>${theSumApr[0]}</strong></td>
						<td><strong>${theSumApr[1]}</strong></td>
						<td><strong>${theSumApr[2]}</strong></td>
						<td><strong>${theSumApr[3]}</strong></td>
						<td><strong>${theSumApr[4]}</strong></td>
						<td><strong>${theSumApr[5]}</strong></td>
						<td><strong>${theSumApr[6]}</strong></td>
						<td><strong>${theSumApr[0]+theSumApr[1]+theSumApr[2]+theSumApr[3]+theSumApr[4]+theSumApr[5]+theSumApr[6]}</strong></td>
						<td colspan="2"></td>
					</tr>
