	...
	<div class="form-container">
		...
		<div class="row">
			<div class="col-md-4 col-sm-4 col-xs-12">
				<div class="form-group">
					...
					<input type="text" class="form-control" id="incomings" value="${theUser.maySalary}">
				</div>
			</div>
			<div class="col-md-4 col-sm-4 col-xs-12">
				<div class="form-group">
					<label>Total outgoings:</label> 
					<input type="text" class="form-control" id="outgoings" value="${theSumMay[0]+theSumMay[1]+theSumMay[2]+theSumMay[3]+theSumMay[4]+theSumMay[5]+theSumMay[6]}">
				</div>
			</div>
			<div class="col-md-4 col-sm-4 col-xs-12">
				<div class="form-group">
					<label>Save Up:</label> 
					<input type="text" class="form-control" style="color: red; font-weight: bold" id="result" value="${theUser.maySalary-theSumMay[0]-theSumMay[1]-theSumMay[2]-theSumMay[3]-theSumMay[4]-theSumMay[5]-theSumMay[6]}">
				</div>
			</div>
		</div>
		</c:forEach>
        ...
        <div class="row">
	        ...
	        <div class="col-md-2 col-sm-2 col-xs-12">
		        <div class="form-group">
             		<input type="button" value="Add new expends" class="btn btn-success btn-block" onclick="window.location.href='showFormForAddExpendsMay'; return false;"/>
		        </div>
	        </div>
		</div>
		...
		<div class="table-responsive center">
			<table class="table table-hover table-sm table-bordered">
				...
				<c:forEach var="tempMayExp" items="${mayExpends}">
				
					<!-- construct an "update" link with row id -->
					<c:url var="updateLink" value="/user/showExpForUpdateMay">
						<c:param name="rowId" value="${tempMayExp.id}" />
					</c:url>
					
					<!-- construct an "delete" link with row id -->
					<c:url var="deleteLink" value="/user/deleteRowMay">
						<c:param name="rowId" value="${tempMayExp.id}" />
					</c:url>
				
					<tr>
						<td>${tempMayExp.id}</td>
						<td>${tempMayExp.date}</td>
						<td>${tempMayExp.eating}</td>
						<td>${tempMayExp.mobile}</td>
						<td>${tempMayExp.flat}</td>
						<td>${tempMayExp.fuel}</td>
						<td>${tempMayExp.tickets}</td>
						<td>${tempMayExp.payment}</td>
						<td>${tempMayExp.other}</td>
						<td><strong>${tempMayExp.total=tempMayExp.eating+tempMayExp.mobile+tempMayExp.flat+tempMayExp.fuel+tempMayExp.tickets+tempMayExp.payment+tempMayExp.other}</strong></td>
					...
					<tr class="background">
						...
						<td><strong>${theSumMay[0]}</strong></td>
						<td><strong>${theSumMay[1]}</strong></td>
						<td><strong>${theSumMay[2]}</strong></td>
						<td><strong>${theSumMay[3]}</strong></td>
						<td><strong>${theSumMay[4]}</strong></td>
						<td><strong>${theSumMay[5]}</strong></td>
						<td><strong>${theSumMay[6]}</strong></td>
						<td><strong>${theSumMay[0]+theSumMay[1]+theSumMay[2]+theSumMay[3]+theSumMay[4]+theSumMay[5]+theSumMay[6]}</strong></td>
						<td colspan="2"></td>
					</tr>
