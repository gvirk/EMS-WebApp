	...
	<div class="form-container">
		...
		<div class="row">
			<div class="col-md-4 col-sm-4 col-xs-12">
				<div class="form-group">
					...
					<input type="text" class="form-control" id="incomings" value="${theUser.julySalary}">
				</div>
			</div>
			<div class="col-md-4 col-sm-4 col-xs-12">
				<div class="form-group">
					<label>Total outgoings:</label> 
					<input type="text" class="form-control" id="outgoings" value="${theSumJul[0]+theSumJul[1]+theSumJul[2]+theSumJul[3]+theSumJul[4]+theSumJul[5]+theSumJul[6]}">
				</div>
			</div>
			<div class="col-md-4 col-sm-4 col-xs-12">
				<div class="form-group">
					<label>Save Up:</label> 
					<input type="text" class="form-control" style="color: red; font-weight: bold" id="result" value="${theUser.julySalary-theSumJul[0]-theSumJul[1]-theSumJul[2]-theSumJul[3]-theSumJul[4]-theSumJul[5]-theSumJul[6]}">
				</div>
			</div>
		</div>
		</c:forEach>
        ...
        <div class="row">
	        ...
	        <div class="col-md-2 col-sm-2 col-xs-12">
		        <div class="form-group">
             		<input type="button" value="Add new expends" class="btn btn-success btn-block" onclick="window.location.href='showFormForAddExpendsJul'; return false;"/>
		        </div>
	        </div>
		</div>
		...
		<div class="table-responsive center">
			<table class="table table-hover table-sm table-bordered">
				...
				<c:forEach var="tempJulExp" items="${julyExpends}">
				
					<!-- construct an "update" link with row id -->
					<c:url var="updateLink" value="/user/showExpForUpdateJul">
						<c:param name="rowId" value="${tempJulExp.id}" />
					</c:url>
					
					<!-- construct an "delete" link with row id -->
					<c:url var="deleteLink" value="/user/deleteRowJul">
						<c:param name="rowId" value="${tempJulExp.id}" />
					</c:url>
				
					<tr>
						<td>${tempJulExp.id}</td>
						<td>${tempJulExp.date}</td>
						<td>${tempJulExp.eating}</td>
						<td>${tempJulExp.mobile}</td>
						<td>${tempJulExp.flat}</td>
						<td>${tempJulExp.fuel}</td>
						<td>${tempJulExp.tickets}</td>
						<td>${tempJulExp.payment}</td>
						<td>${tempJulExp.other}</td>
						<td><strong>${tempJulExp.total=tempJulExp.eating+tempJulExp.mobile+tempJulExp.flat+tempJulExp.fuel+tempJulExp.tickets+tempJulExp.payment+tempJulExp.other}</strong></td>
					...
					<tr class="background">
						...
						<td><strong>${theSumJul[0]}</strong></td>
						<td><strong>${theSumJul[1]}</strong></td>
						<td><strong>${theSumJul[2]}</strong></td>
						<td><strong>${theSumJul[3]}</strong></td>
						<td><strong>${theSumJul[4]}</strong></td>
						<td><strong>${theSumJul[5]}</strong></td>
						<td><strong>${theSumJul[6]}</strong></td>
						<td><strong>${theSumJul[0]+theSumJul[1]+theSumJul[2]+theSumJul[3]+theSumJul[4]+theSumJul[5]+theSumJul[6]}</strong></td>
						<td colspan="2"></td>
					</tr>
